...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("preserves compacted usage when completion follows an older assistant frame", () => {
    ...
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      // Each compact boundary now also clears the compaction session overlay.
  ...
  it.effect("reports session heartbeats only while a turn owns them", () => {
    ...
    return Effect.gen(function* () {
      ...
      // Compaction outlives the turn that requested it: the CLI clears its
      // status once the work lands, well after the turn closed. With no turn
      // to own the heartbeat, the cleared status must land the session on
      // ready so the pair self-clears.
      harness.query.emit({
        type: "system",
        subtype: "status",
        status: null,
      ...
      assert.deepEqual(heartbeats, ["compacting:status:compacting", "ready:status:active"]);
      runtimeEventsFiber.interruptUnsafe();
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
