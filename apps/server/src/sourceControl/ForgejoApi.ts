...
// `fj` keys its login store and `fj auth list` output by bare hostname, while HTTP remote URLs
// can carry a `:port`. Match host identities port-insensitively while preserving HTTP ports for
// API base URLs.
...
export function parseForgejoRemoteUrl(remoteUrl: string): ForgejoRepositoryLocator | null {
  ...
  if (scp?.[1] && scp[2]) {
    ...
    const parsed = parseRepoPath(scp[2]);
    // SSH remotes don't imply an API scheme; default to https.
    return parsed ? { host, scheme: "https", ...parsed } : null;
  }
  try {
    const url = new URL(trimmed);
    if (!["http:", "https:", "ssh:", "git:"].includes(url.protocol) || url.host.length === 0) {
      return null;
    }
    const parsed = parseRepoPath(url.pathname);
    const scheme = url.protocol === "http:" ? "http" : "https";
    // An SSH URL's port belongs to the Git transport, not the Forgejo HTTP API. HTTP(S)
    // remotes do carry the API authority, so retain their custom ports.
    const host = url.protocol === "http:" || url.protocol === "https:" ? url.host : url.hostname;
    return parsed ? { host: host.toLowerCase(), scheme, ...parsed } : null;
  } catch {
    return null;
  }
}
...
export function parseForgejoRepositorySpec(
  ...
  if (SCP_REMOTE_PATTERN.test(trimmed) || /^[a-z][a-z0-9+.-]*:\/\//iu.test(trimmed)) {
...
function normalizeRepositoryCloneUrls(
  ...
  locator: Pick<ForgejoRepositoryLocator, "host" | "scheme">,
): SourceControlRepositoryCloneUrls {
  const httpUrl =
    raw.clone_url?.trim() ||
    raw.html_url?.trim() ||
    `${locator.scheme}://${locator.host}/${raw.full_name}.git`;
  // SCP-style clone URLs cannot encode a port. Forgejo normally returns `ssh_url`; when it
  // does not, fall back to the hostname and the standard SSH port instead of producing an
  // invalid `git@host:http-port:owner/repo.git` URL.
  const sshUrl = raw.ssh_url?.trim() || `git@${stripHostPort(locator.host)}:${raw.full_name}.git`;
...
function requestError(operation: string): ForgejoApiError {
  return new ForgejoApiError({
    operation,
    detail: "Failed to send the Forgejo request.",
  });
}
...
export const make = Effect.gen(function* () {
  ...
  const executeJson = <S extends Schema.Top>(
    ...
    withAuth(host, request.pipe(HttpClientRequest.acceptJson)).pipe(
      ...
      Effect.mapError((cause) => (isForgejoApiError(cause) ? cause : requestError(operation))),
  ...
  const resolveRepository = Effect.fn("ForgejoApi.resolveRepository")(function* (input: {
    ...
    if (input.repository !== undefined) {
      ...
      return yield* new ForgejoApiError({
        ...
        detail:
          contextHost === null && hosts.length === 0
            ? "No authenticated Forgejo instance is configured; run `fj auth login <host>` or specify a full repository URL."
            : contextHost === null && hosts.length > 1
              ? "Multiple Forgejo instances are configured; specify the repository as host/owner/repository or use a full repository URL."
              : "Forgejo repositories must be specified as owner/repository, host/owner/repository, or a full repository URL.",
      });
    }
      ...
      resolveRepository(input).pipe(
        Effect.flatMap((locator) =>
          getRepositoryFromLocator(locator).pipe(
            Effect.map((raw) => normalizeRepositoryCloneUrls(raw, locator)),
          ),
        ),
      ),
      ...
      Effect.gen(function* () {
        ...
        if (!locator) {
          return yield* new ForgejoApiError({
            ...
            detail:
              hosts.length === 0
                ? "No authenticated Forgejo instance is configured; run `fj auth login <host>` or specify the repository as host/owner/repository."
                : hosts.length > 1
        ...
        // `fj` browser logins do not persist the account name, so ask Forgejo for the canonical
        // login before deciding between the personal and organization creation endpoints.
        ...
        const isOwnAccount = user.login.toLowerCase() === locator.owner.toLowerCase();
        ...
        return normalizeRepositoryCloneUrls(raw, locator);
      }),
      ...
      Effect.gen(function* () {
        ...
        if (
          ...
          if (primaryRemote) {
            ...
            const cloneUrls = normalizeRepositoryCloneUrls(raw, destination);
