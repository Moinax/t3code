...
export function parseForgejoAuthHosts(
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    const accountHost = /^([^@\s]+)@([a-z0-9][a-z0-9.-]*(?::\d+)?)$/iu.exec(trimmed);
    if (accountHost?.[1] && accountHost[2]) {
      entries.push({ account: accountHost[1], host: accountHost[2].toLowerCase() });
      continue;
    }
    if (/^[a-z0-9][a-z0-9.-]*(?::\d+)?$/iu.test(trimmed)) {
      entries.push({ host: trimmed.toLowerCase() });
    }
  }
  return entries;
}
...
export const discovery = {
  ...
  installHint:
    "Install the Forgejo CLI (`fj`) from https://codeberg.org/forgejo-contrib/forgejo-cli and run `fj auth login <host>`. The same CLI works against Gitea-compatible instances.",
